...
with col6:
    if game.bot.score == []:
        ...
        st.image(game.bot.score[0].image, caption = "Presi")
    st.image("static/images/VUOTO.png",width=89) # Width per motivi estetici   
    if game.player.score == []:
        st.image("static/images/VUOTO.png",width=89) # Width per motivi estetici      
    else:
        st.image(game.player.score[0].image, caption = "Presi")
...
if st.session_state['turno'] == 1 :
    if st.button("io"):
        game.player.score.extend([game.table.first_card,game.table.second_card])
        st.rerun()
    if st.button("bot"):
        game.bot.score.extend([game.table.first_card,game.table.second_card])
        st.rerun()
